...
def test_model(model,inputimg_path,targetimg_path,input_age,target_age,output,batch_size,l1_weight: float = 1.0, lpips_weight: float =1.0, adv_weight: float = 0.05):   
            ...
            transform = transforms.Compose([transforms.Resize(size=(1024, 1024)),
                                            transforms.ToTensor(),
            ...
            if (targetimg_path != None):
                ...
                save_image(concat_out_img,os.path.join("./results/",f'{img_id}_iage_{input_age}_tage_{target_age}_modelout_1024.png'))
            else:
...
def get_args():
    ...
    parser.add_argument('--model', '-m',metavar="FILE", default="checkpoints/FRAN_UNet_Sun_31Dec2023_152011_epoch60.pth",help='Specify the file in which the model is stored')
    ...
    parser.add_argument('--inputimage', '-i', metavar='INPUT', dest="input", help='Filename of input image', default="input/Yan.JPG")
    parser.add_argument('--inputage', '-ia', metavar='INPUTAGE', dest="inputage", help='Input age', default=40)
    parser.add_argument('--targetage', '-ta', metavar='TARGETAGE',dest="targetage", help='Target age', default=20)
